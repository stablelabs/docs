...
:::note
**개념:** 구독 모델, 트레이드오프, 카드 등록 청구와의 비교에 대해서는 [구독 청구](/ko/reference/subscriptions)를 참조하세요.
:::
...
:::note
이 컨트랙트는 테스트 목적의 참조 구현으로 제공됩니다. 위임 컨트랙트는 구독자의 EOA에 대한 완전한 실행 권한을 가지므로, 프로덕션에서는 감사 및 검증된 컨트랙트를 사용하세요. EIP-7702 위임 및 보안에 대한 자세한 내용은 [EIP-7702](/ko/explanation/eip-7702)를 참조하세요.
:::
...
- [**구독 청구 개념**](/ko/reference/subscriptions) — 풀 기반 청구 모델을 이해하세요.
- [**계정 추상화**](/ko/how-to/account-abstraction) — 일괄 결제, 지출 한도, 세션 키가 하나의 위임 하에서 어떻게 결합되는지 확인하세요.
- [**EIP-7702 개념**](/ko/explanation/eip-7702) — 이를 가능하게 하는 위임 모델을 검토하세요.

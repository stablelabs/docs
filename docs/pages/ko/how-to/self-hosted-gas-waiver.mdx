...
:::note
**개념:** 가스 면제가 무엇이며 왜 존재하는지에 대해서는 [가스 면제](/ko/explanation/gas-waiver)를 참조하세요. 전체 프로토콜 사양(래퍼 트랜잭션 메커니즘, 권한 부여, 정책 확인, 실행 의미론, 보안 모델)에 대해서는 [가스 면제 프로토콜](/ko/reference/gas-waiver-api)을 참조하세요.
:::

호스팅된 Waiver Server API 통합 경로에 대해서는 [가스 비용 없는 트랜잭션 활성화](/ko/how-to/integrate-gas-waiver)를 참조하세요.
...
- [**가스 면제 개념**](/ko/explanation/gas-waiver) — 직접 운영하기 전에 메커니즘을 이해하세요.
- [**가스 면제 프로토콜**](/ko/reference/gas-waiver-api) — 마커 라우팅, 권한 부여, 실행 의미론에 대한 전체 프로토콜 사양을 참조하세요.
- [**가스 비용 없는 트랜잭션 활성화**](/ko/how-to/integrate-gas-waiver) — 자체 호스팅 대신 호스팅된 Waiver Server API를 사용하세요.

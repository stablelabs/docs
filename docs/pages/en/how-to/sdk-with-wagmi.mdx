...
The same `stable` instance handles bridge and swap. Fetch the quote in an effect or a `useQuery`, then execute on click. Catch `StableQuoteError`: LI.FI's DEX coverage on Stable is still limited, and pairs without a route throw "No available quotes for the requested transfer".
